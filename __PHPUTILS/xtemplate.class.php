...
if ( !class_exists('XTemplate') ){
	class XTemplate {
		...
		public function setup ($add_outer = false) {
			...
			$this->assign('PIR_DOMAIN', PIR_DOMAIN);
			$this->assign('PIRPATH', PIRPATH);
			$this->assign('PBA3_DOMAIN', PBA3_DOMAIN);
			$this->assign('PBA3PATH', PBA3PATH);
			$this->assign('JELENLEGI_DOMAIN', JELENLEGI_DOMAIN);*/
		...
		*/
		...
		private function getS11tpl() {
			$url = str_replace(strtolower(PIRPATH), 'pir/' , strtolower($this->filename));
			$url = str_replace(strtolower(PBA3PATH), 'pba3.1/' , strtolower($url));
			if((int)_REQUESTHandler::getData('s', DATATYPE::EGESZ_SZAM, _REQUESTHandler::GET) === 69){
				dg::n('tpl_betoltes', $url);
			}
			$sql = "SELECT s.`statusz` FROM `egyeb`.`s11_teszt` s WHERE s.`url` = :url LIMIT 1;";
			if(DB::runSQL($sql, array('url'=>array($url, DATATYPE::STRING)), DB::FETCH_COLUMN)){
				dg::n('s11 tpl_betoltes', $url);
				$ch = curl_init();
				$s11 = 'https://pba.super11.hu/';
				if (StringKezelo::szerepelASzovegben($s11 . $url, 'https:')) {
					curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
					curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
				}
				curl_setopt($ch, CURLOPT_URL, $s11 . $url);
			...
			return $this->_r_getfile($this->filename);
